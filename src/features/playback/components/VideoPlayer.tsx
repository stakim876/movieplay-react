...
import { fetchTVSeason } from "@/core/api/tmdb";
import PlaybackScopeNotice from "@/shared/ui/PlaybackScopeNotice";
import type { PlaybackSource } from "@/features/playback/lib/playbackSource";
...
export default function VideoPlayer({
  ...
  movie: Record<string, unknown> & { id: number; title?: string; name?: string };
  mediaType?: string;
  ...
  const [showNextUp, setShowNextUp] = useState<boolean>(false);
  const [nextUpSeconds, setNextUpSeconds] = useState(8);
  const [resolvedNextUrl, setResolvedNextUrl] = useState<any>(null);
  const [resolvedPrevUrl, setResolvedPrevUrl] = useState<any>(null);
  
  const navigate = useNavigate();
  const { updateWatchProgress, getWatchProgress } = useWatchHistory();
  
  const controlsTimeoutRef = useRef<any>(null);
  const nextUpTimerRef = useRef<any>(null);
  ...
  useEffect(() => {
    // 에피소드 빠르게 바꿀 때, 늦게 도착한 API 응답이 화면을 덮어쓰지 않게 막음
    let cancelled = false;
  ...
  // <video>는 ref로 직접 currentTime 설정 (state로 재생 위치 관리하면 초당 수십 번 리렌더)
  useEffect(() => {
    if (!movie) return;
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      // timeupdate는 초당 여러 번 옴 → 5초마다 한 번만 시청 기록 저장
    ...
    // 영상 바뀌거나 컴포넌트 사라질 때 이벤트 리스너 제거 (안 하면 setState 경고)
    return () => {
  ...
  const defaultVideoUrl =
    videoUrl || (playbackSource.mode === "demo-sample" ? playbackSource.url : "");
  ...
  if (playbackSource.mode === "youtube-trailer") {
    return (
      <div className="video-player-container video-player-container--trailer" ref={containerRef}>
        <div className="video-player-controls video-player-controls--trailer">
        ...
        <div className="player-scope-notice">
          <PlaybackScopeNotice
            label={playbackSource.label}
            description={playbackSource.description}
            compact
          />
        </div>
        ...
        <iframe
          ...
          src={`https://www.youtube-nocookie.com/embed/${playbackSource.youtubeId}?autoplay=1&rel=0&modestbranding=1`}
