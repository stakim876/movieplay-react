...
import { fetchSearchResults } from "@/core/api/tmdb";
import "@/styles/components/components.css";

const PROFILE_KEY = "mp_active_profile_v1";
const LOGO_SRC = "/assets/movieplay-logo.svg";
...
export default function Header({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const data = await fetchSearchResults(trimmed, "movie");
      ...
      navigate(`${ROUTES.search}?query=${encodeURIComponent(trimmed)}`);
      setQuery("");
    } catch (error) {
    ...
    <header className={`header ${scrolled ? "scrolled" : ""}`}>
      <div className="header-content">
        ...
        <nav className={`nav-links ${mobileMenuOpen ? "mobile-open" : ""}`}>
          ...
          <Link
            to={ROUTES.browse}
            className={`nav-item ${isActive(ROUTES.browse) ? "active" : ""}`}
          >
            둘러보기
          </Link>
          ...
          <Link
            to={ROUTES.newHot}
            className={`nav-item ${isActive(ROUTES.newHot) ? "active" : ""}`}
          >
            New & Hot
          </Link>
