import { forwardRef, useEffect, useMemo, useRef, useState, type ReactNode, type UIEvent } from "react";
...
// [면접] forwardRef = 부모가 이 스크롤 div의 DOM에 직접 접근할 수 있게 ref 전달
// 내부에서 쓰는 ref와 부모가 넘긴 ref를 callback ref로 합침
const HorizontalScroller = forwardRef<HTMLDivElement, HorizontalScrollerProps>(function HorizontalScroller(
  ...
  const setForwardedRef = (node: HTMLDivElement | null) => {
    ...
    // ref는 { current } 객체형 / (node) => {} 함수형 두 가지가 있어서 분기
  ...
  // 드래그 중인지 여부는 리렌더 필요 없음 → useRef에 저장 (state 쓰면 드래그마다 렌더 폭발)
  ...
  useEffect(() => {
    ...
    // 스크롤 컴포넌트는 관찰자·타이머가 많아서, 언마운트 시 전부 해제 필수
    return () => {
